...
- **Voting power follows tier configuration.** Each tier contributes `balance × (useVotingUnits ? customVotingUnits : storedTier.price)` (`JB721TiersHookStore.sol:380-383, 436-440`). The checkpoint module reads tier voting units from the store on every transfer (`JB721Checkpoints.sol:122-126`).
...
- `JB721Checkpoints.initialize(hookAddress)` (`JB721Checkpoints.sol:103-107`). One-shot; the implementation pre-sets `hook = address(1)` in its constructor (`JB721Checkpoints.sol:62`).
- `JB721Checkpoints.onTransfer(from, to, tokenId)` (`JB721Checkpoints.sol:114-127`). Hook-only. Pushes an owner checkpoint (for `from != address(0)`) and moves voting units between `from` and `to` based on `STORE.tierOfTokenId(...).votingUnits`.
- `JB721Checkpoints.delegate(delegatee, tokenIds[])` (`JB721Checkpoints.sol:75-98`). Delegates voting power; also enrolls each token by writing an owner checkpoint, but only if the caller currently owns the token.
- `JB721Checkpoints.ownerOfAt(tokenId, blockNumber)` (`JB721Checkpoints.sol:139-153`). Returns `address(0)` for tokens that have never been enrolled or transferred — those tokens are ineligible for snapshot-based distribution.
...
| Checkpoints `onTransfer` hook-only | `src/JB721Checkpoints.sol` | 114-127 |
