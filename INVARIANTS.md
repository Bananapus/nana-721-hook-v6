...
- **`getPastTierVotingUnits` is the per-tier analogue of `getPastTotalSupply`.** `JB721Checkpoints` maintains a checkpointed per-tier eligible-voting-units trace (`_tierEligibleUnitsOf`) that distributors read as the exact historical denominator for tier-scoped reward pots (`JB721Checkpoints.sol:179-181`). It moves ONLY when a token's eligibility changes: it increments by the token's tier voting units when the token first gains an owner checkpoint — enrollment via `delegate(address, uint256[])` (`JB721Checkpoints.sol:95-117`) or the `from != address(0)` first-transfer branch of `onTransfer` (`JB721Checkpoints.sol:161-163`) — and decrements on burn (`JB721Checkpoints.sol:154-160`). **Mints write nothing** (the `from == address(0)` path is skipped, `JB721Checkpoints.sol:145-147`), so the mint path adds zero checkpoint gas and the trace tracks exactly the set of tokens `ownerOfAt` reports as eligible.
- **`getPastTotalActiveVotes` tracks delegated participation, not owner eligibility.** `JB721Checkpoints` maintains `_activeSupplyCheckpoints`, the total voting units currently held by accounts with nonzero delegates. Delegating held units increases the active total, clearing delegation decreases it, and transfers move active units only when the sender and receiver differ in delegated status. A self-delegated holder who transfers a token into undelegated AMM custody makes those units inactive; if the token returns while the holder's delegation remains set, those units become active again. This active total is not the tier-scoped reward denominator.
...
- `JB721Checkpoints.onTransfer(from, to, tokenId)` (`JB721Checkpoints.sol:139-168`). Hook-only. For `from != address(0)` it pushes an owner checkpoint and updates the per-tier eligible-units trace (add on a never-enrolled token's first transfer, subtract on burn of an eligible token), then moves voting units between `from` and `to`. On mint (`from == address(0)`) it writes neither eligibility trace — the mint path adds no checkpoint gas.
- `JB721Checkpoints.delegate(delegatee, tokenIds[])` (`JB721Checkpoints.sol:95-123`). Delegates voting power; also enrolls each token by writing an owner checkpoint and adding its tier voting units to the per-tier eligible-units trace, but only if the caller currently owns the token and the token has no checkpoint yet.
- `JB721Checkpoints.getPastTierVotingUnits(tierId, blockNumber)` (`JB721Checkpoints.sol:179-181`). The per-tier analogue of `getPastTotalSupply`: returns the tier's total eligible voting units at a past block, where "eligible" means tokens with an owner checkpoint (enrolled or transferred at least once). Distributors use it as the denominator for tier-scoped reward pots. Mints never contribute to it.
- `JB721Checkpoints.getPastTotalActiveVotes(blockNumber)` and `getTotalActiveVotes()` (`JB721Checkpoints.sol:188-197`). Return historical and current totals of voting units held by accounts with nonzero delegates. These totals are for active delegated participation and remain separate from `ownerOfAt` / `getPastTierVotingUnits` reward eligibility.
- `JB721Checkpoints.ownerOfAt(tokenId, blockNumber)` (`JB721Checkpoints.sol:205-219`). Returns `address(0)` for tokens that have never been enrolled or transferred — those tokens are ineligible for snapshot-based distribution.
...
| Checkpoints active delegated vote trace (`getPastTotalActiveVotes`) | `src/JB721Checkpoints.sol` | 70-71, 188-197 |
| Checkpoints per-tier eligible-units trace (`getPastTierVotingUnits`) | `src/JB721Checkpoints.sol` | 60-64, 179-181 |
| Checkpoints mint writes no eligibility trace (no added mint gas) | `src/JB721Checkpoints.sol` | 145-147 |
