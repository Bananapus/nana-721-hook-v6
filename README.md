...
A Juicebox data hook, pay hook, and cash-out hook that lets projects sell tiered NFTs (ERC-721s) with different prices, artwork, and categories. Optionally, holders can burn their NFTs to reclaim funds from the project in proportion to the NFT's price.
...
|----------|-------------|
| [`JB721Hook.sol`](https://github.com/Bananapus/nana-721-hook-v6/blob/main/src/abstract/JB721Hook.sol) | Abstract base for 721 hooks: handles pay/cash out lifecycle, terminal validation, and metadata resolution. |
| [`JB721TiersHook.sol`](https://github.com/Bananapus/nana-721-hook-v6/blob/main/src/JB721TiersHook.sol) | The core tiered NFT pay/cash out hook implementation, extending `JB721Hook`. |
| [`JB721TiersHookDeployer.sol`](https://github.com/Bananapus/nana-721-hook-v6/blob/main/src/JB721TiersHookDeployer.sol) | Exposes a `deployHookFor(...)` function which deploys an NFT hook for a project. |
| [`JB721TiersHookProjectDeployer.sol`](https://github.com/Bananapus/nana-721-hook-v6/blob/main/src/JB721TiersHookProjectDeployer.sol) | Exposes a `launchProjectFor(...)` function which deploys a project with a tiered NFT hook already set up. |
| [`JB721TiersHookStore.sol`](https://github.com/Bananapus/nana-721-hook-v6/blob/main/src/JB721TiersHookStore.sol) | Stores and manages data for tiered NFT hooks. |
...
If you're using `forge` to manage dependencies, you'll also need to install `nana-721-hook-v6`'s dependencies. With `libs = ["node_modules", "lib"]` in `foundry.toml`, Foundry auto-resolves import paths -- no `remappings.txt` needed.

## Develop

`nana-721-hook-v6` uses [npm](https://www.npmjs.com/) (version >=20.0.0) for package management and [Foundry](https://github.com/foundry-rs/foundry) for builds and tests.
...
`nana-721-hook-v6` manages deployments with [Sphinx](https://www.sphinx.dev). To run the deployment scripts, install the npm `devDependencies` with:
...
This example deploys `nana-721-hook-v6` to the Sepolia testnet using the specified private key. You can configure new networks in `foundry.toml`.
...
- The hook iterates over the full tier set in several read paths (`balanceOf`, `totalCashOutWeight`, `tierIdOfToken`). Projects with many active tiers may encounter high gas costs or hit block gas limits during cash outs and governance reads.
- Tier prices are fixed at creation time. If the payment token's value changes significantly, NFT prices cannot be adjusted without removing and re-adding tiers (which resets supply counters).
- If `PRICES` is set to the zero address and a payment arrives in a currency that differs from a tier's price currency, the payment is silently ignored -- no NFTs are minted and no error is raised.
- NFT cash outs and project token cash outs are mutually exclusive. Enabling `useDataHookForCashOut` disables fungible token cash outs entirely, which may surprise token holders.
- Pending reserved NFTs are included in the cash out weight denominator, diluting per-NFT reclaim value for existing holders before the reserves are actually minted.
- Per-tier splits route funds outside the project treasury. If splits are misconfigured, funds may be sent to unintended recipients with no on-chain recovery mechanism.
- The `cleanTiers()` function must be called manually after removing tiers. Until it is called, removed tiers still consume gas during iteration.
- Pool key assignments via the hook's `PRICES` contract are immutable once set. If a price feed becomes stale or is deprecated, a new hook deployment is required.
