...
Tiered ERC-721 NFT hook for Juicebox V5 that mints NFTs when a project is paid and optionally allows NFT holders to burn them to reclaim project funds proportional to tier price.
...
|------------|--------|----------|
| `@bananapus/core-v5` | `IJBDirectory`, `IJBRulesets`, `IJBPrices`, `IJBController`, `IJBTerminal`, `JBRuleset`, `JBRulesetMetadata`, `JBAfterPayRecordedContext`, `JBBeforeCashOutRecordedContext`, etc. | Terminal validation, ruleset metadata, pricing, payment/cash-out contexts |
| `@bananapus/ownable-v5` | `JBOwnable` | Project-based ownership for the hook (ownership can be transferred to a project NFT) |
| `@bananapus/permission-ids-v5` | `JBPermissionIds` | Permission IDs: `ADJUST_721_TIERS`, `MINT_721`, `SET_721_METADATA`, `SET_721_DISCOUNT_PERCENT`, `QUEUE_RULESETS`, `SET_TERMINALS` |
| `@bananapus/address-registry-v5` | `IJBAddressRegistry` | Registering deployed hook clones |
...
- `script/Deploy5_1.s.sol` has a pre-existing build error (imports `core.rulesets5_1` which does not exist). Use `forge test --skip "script/*"` to avoid it.
...
```solidity
import {IJB721TiersHookProjectDeployer} from "@bananapus/721-hook-v5/src/interfaces/IJB721TiersHookProjectDeployer.sol";
