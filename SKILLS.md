...
| `JB721TiersHookLib` (library) | External library called via DELEGATECALL from the hook. Handles tier adjustments (`adjustTiersFor`), split amount calculation (`calculateSplitAmounts`), fault-tolerant split fund distribution (`distributeAll` using `this.executeSplitPayout()` + try/catch), price normalization (`normalizePaymentValue`), and token URI resolution (`resolveTokenURI`). Extracted to stay within EIP-170 contract size limit. |
...
| `executeSplitPayout(split, token, amount, projectId, groupId, decimals)` | `JB721TiersHook` | External payable, self-call only (`require(msg.sender == address(this))`). Executes a single split payout following priority: `split.hook` > `split.projectId` > `split.beneficiary`. Called by `JB721TiersHookLib._trySplitPayout` via `this.executeSplitPayout()` so try/catch can wrap the external call. If it reverts, all state changes inside roll back atomically. Mirrors nana-core-v6's `JBMultiTerminal.executePayout` pattern. |
| `distributeAll(directory, splits, projectId, hookAddress, token, amount, decimals, encodedSplitData)` | `JB721TiersHookLib` | Called in `afterPayRecordedWith`. Decodes per-tier amounts, looks up each tier's splits from `JBSplits` by group ID (`hookAddress | (tierId << 160)`). Distributes to split recipients via `_trySplitPayout`, which wraps `this.executeSplitPayout()` in try/catch — a single reverting split does NOT block other splits in the same tier or subsequent tiers. Failed splits emit `SplitPayoutReverted` and their funds stay in leftoverAmount, which routes to the project's balance via `addToBalance`. |
...
- In `afterPayRecordedWith`, `distributeAll` distributes forwarded funds to each tier's split group recipients. Each split payout is wrapped in try/catch via `this.executeSplitPayout()` — if any individual split recipient reverts, its funds stay in leftoverAmount and route to the project's balance, while other splits proceed normally. A `SplitPayoutReverted` event is emitted for each failed split with the revert reason. This mirrors nana-core-v6's `JBMultiTerminal.executePayout` pattern.
