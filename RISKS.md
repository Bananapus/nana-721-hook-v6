...
- **Clone initialization is one-shot, atomic.** `initialize()` guards via `PROJECT_ID != 0` (for clones) and `bytes(ERC721.name()).length != 0` (for the implementation contract itself). The `ERC721.name()` check prevents the implementation from being initialized even though its `PROJECT_ID` is zero. Deployer contracts call deploy+initialize in a single transaction, preventing front-running. Ownership transfers to `_msgSender()` at the end of `initialize`.
